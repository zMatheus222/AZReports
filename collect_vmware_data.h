...
inline vector<QString> collect_vmware_data(QString item, vector<QString> wiki_vmwares){
    ...
    vector<QString> empty = {"collect_vmware_data::Empty"};
    ...
    */
    ...
    regex rgx_vm_machine("^M[áa-zA-Z]+\\s([0-9]+)$");
    ...
    for(QString& line : wiki_vmwares){
        ...
        //se estivermos em uma linha onde possui por exemplo: "Máquina 78", ative este if para salvar a maquina, se for outro tipo de linha não salvamos, e o valor da ultima maquina encontrada fica armazenado.
        if(rgx_sch_matched_machine == true){
            //salvar numero da maquina na var vmware_machine
            vmware_machine = smatch_vm_machine[1];
            qDebug() << "if(rgx_sch_matched_machine == true)::vmware_machine: " << vmware_machine;
            ...
            qDebug() << "entered else if(rgx_sch_matched_vmware == true)";
            ...
            for(int i = 1; i <= smatch_count; i++){
                ...
                if(conv_item == smatch_vmware[i]){
                    ...
                    for(const auto line : smatch_vmware){
                        ...
                        qDebug() << "for(const auto line : smatch_vmware): " << &line;
